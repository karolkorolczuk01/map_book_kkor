...
def edit_user():
    ...
    buttn_dodaj_obiekt.configure(text='Zapisz', command=lambda: update_user(i))
...
def update_user(i):
    ...
    show_users()
    buttn_dodaj_obiekt.configure(text='Dodaj', command=add_user)  # zmiana właściwosci przycisku
...
buttn_dodaj_obiekt = Button(ramka_formularz, text='Dodaj', command=add_user)
buttn_dodaj_obiekt.grid(row=5, columnspan=2)

# ramka_szczegoly_obiektu
label_pokaz_szczegoly = Label(ramka_szczegoly_obiektow, text='Szczególy użytkownika: ')
